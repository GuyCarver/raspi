...
ADDRESS_0 = 0x68                                # Default address
ADDRESS_1 = 0x69                                # Address when ADD pin set high
...
#     high = self.i2c.read_byte_data(self._address, aLoc)
#     low = self.i2c.read_byte_data(self._address, aLoc + 1)
#
#     res = (high << 8) | low
#
#     if(res > 32768):
...
#     return res
  ...
  def _readdata( self, aLoc, aLen ):
    ''' read aLen bytes starting at aLoc '''
    return self.i2c.read_i2c_block_data(self._address, aLoc, aLen)

  def _readbits( self, aLoc, aBit, aLen ) :
    v = self._read8(aLoc)
    b = (v >> (aBit - aLen + 1)) & ((1 << aLen) - 1)
    return b

  def _writebits( self, aLoc, aBit, aLen, aValue ) :
    v = self._read8(aLoc)
    mask = ((1 << aLen) - 1) << (aBit - aLen + 1)
    tmp = (v << (aBit - aLen + 1)) & mask #shift data into correct position
    tmp &= ~mask
    tmp |= aValue
    self._write8(tmp)

  def _write8( self, aLoc, aValue ):
    """Write 8 bit integer aVal to given address aLoc."""
    self.i2c.write_byte_data(self._address, aLoc, aValue)

  def _write16( self, aLoc, aValue ):
    """Write 8 bit integer aVal to given address aLoc."""
    self.i2c.write_word_data(self._address, aLoc, aValue)

  def _writedata( self, aLoc, aBuffer ):
    """Write buffer to given location."""
    self.i2c.write_i2c_block_data(self._address, aLoc, aBuffer)

  def getrate( self ) :
    return self._read16(RA_SMPLRT_DIV)

  def setrate( self, aRate ) :
    self._write16(RA_SMPLRT_DIV, aRate)
  ...
  def getacceltemprot( self ) :
    v = self._readdata(RA_ACCEL_XOUT_H, 14)
    return [(v[i] << 8) | v[i + 1] for i in range(0, 14, 2)]

  def getacceleration( self ) :
    v = self._readdata(RA_ACCEL_XOUT_H, 6)
    return [(v[i] << 8) | v[i + 1] for i in range(0, 6, 2)]

  def getrotation( self ) :
    v =self._readdata(RA_GYRO_XOUT_H, 6)
    return [(v[i] << 8) | v[i + 1] for i in range(0, 6, 2)]
